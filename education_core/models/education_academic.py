...
class EducationAcademic(models.Model):
    ...
    current_year = fields.Boolean('Current Year', default=True,
                            help="If unchecked, it will allow be The current year.")
    ...
    def _onchange_current_year(self):

        if self.current_year:
            years = self.env['education.academic.year'].search([('id', '!=', self.env.context.get('active_id'))])
            for year in years:
